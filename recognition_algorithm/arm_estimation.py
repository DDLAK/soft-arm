...
with mp_pose.Pose(
    ...
    while cap.isOpened():
        ...
        angle1 = calculate_angle(lm[N_LEFT_SHOULDER], lm[N_LEFT_ELBOW], lm[N_LEFT_WRIST])
        angle2 = calculate_angle(lm[N_RIGHT_SHOULDER], lm[N_LEFT_SHOULDER], lm[N_LEFT_ELBOW])
        print("angle1 = {0}, angle2 = {1}".format(angle1, angle2))

        # Judge if it's an arm movement paradigm.
        if 165 < angle1 < 195 and 165 < angle2 < 195:
            paradigm = 0
        elif 110 < angle1 < 150 and 180 < angle2 < 270 \
                and lm[N_LEFT_WRIST].z < lm[0].z:
            paradigm = 2
        elif 110 < angle1 < 150 and 165 < angle2 < 270 \
                and lm[N_LEFT_WRIST].z > lm[0].z:
            paradigm = 3
        elif 270 < angle1 < 330 and 180 < angle2 < 270 \
                and lm[N_LEFT_WRIST].z < lm[N_LEFT_ELBOW].z < lm[N_LEFT_SHOULDER].z:
            paradigm = 4
        elif 80 < angle1 < 100 and 165 < angle2 < 195:
            paradigm = 1
        else:
            cv2.imshow('Marking Pose', image)
            if cv2.waitKey(5) & 0xFF == 27:
        ...
        image = cv2.line(image, (int(lm[N_LEFT_WRIST].x * 640), int(lm[N_LEFT_WRIST].y * 480)),
                       (int(lm[N_LEFT_ELBOW].x * 640), int(lm[N_LEFT_ELBOW].y * 480)), (0, 0, 255), 5)
        draw_angle(image, lm[N_LEFT_SHOULDER], lm[N_LEFT_ELBOW], lm[N_LEFT_WRIST], angle1)
        draw_angle(image, lm[N_RIGHT_SHOULDER], lm[N_LEFT_SHOULDER], lm[N_LEFT_ELBOW], angle2)
        cv2.putText(image, 'action paradigm {0}'.format(paradigm), (50, 50),
